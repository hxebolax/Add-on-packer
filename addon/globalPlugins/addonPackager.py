...
class GlobalPlugin(globalPluginHandler.GlobalPlugin):
	...
	@script(gesture=None,
		...
		description= _("Shows the add-on packer window"),
...
# Function taken from the add-on emoticons to center the window
	...
	def __init__(self, parent):
		...
		label2 = wx.StaticText(Panel, wx.ID_ANY, label=_("Destination folder:"))
		...
		self.directoryBTN = wx.Button(Panel, wx.ID_ANY, _("&Browse"))
	...
	def onDirectory(self, event):
		...
		dlg = wx.DirDialog(self, _("Select a folder:"),
	...
	def onGenerate(self, event):
		...
		if len(selection) == 0:
			...
			gui.messageBox(_("You need to select at least one add-on to proceed."),
			...
			if self.textDirectory.GetValue() == "":
				...
				gui.messageBox(_("You need to select an output folder to proceed."),
...
class GeneratingThread(Thread):
	...
	def run(self):
		try:
			for i in self.value:
				addonSave = os.path.join(self.directorySave, listAddons[i].manifest["name"] + "_" + listAddons[i].manifest["version"])
